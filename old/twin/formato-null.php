...
/**************************************
 *
 *  CONSIGNACION CAPITANIA	PAG 13  >>>>> MAKE AVO
 *
 **************************************/
    $pdf->addJpegFromFile("abajo.jpg",'0','10','600','');
    $pdf->addJpegFromFile("top.jpg",'5','750','580','');
...
'), 10, array('justification'=>'right'));
$pdf->ezText(utf8_decode('
PARA  LOS  EFECTOS  LEGALES  PROCEDENTES,  HACEMOS  DEL  CONOCIMIENTO  DE  USTED  QUE  EL  B.M.	<b><u> '.$row["vessel"].' </b></u> DE BANDERA <b><u> '.$row["flag"].' </b></u> ZARPARA DE ESTE PUERTO CON DESTINO A: <b><u>'.$row["etfnextport"].'</b></u> ZARPANDO DEL MUELLE <b><u>'.$row["muelle"].'</b></u>  A LAS <b><u>  '.$row["etfhora"].'  </b></u>  HRS. DEL DIA <b><u>  '.$row["etf"].'  </b></u>   ZARPANDO DE ESTE PUERTO CON <b><u>  '.$row["tonstom"].'  </b></u> TONS.  DE CARGA Y  <b><u>  '.$row["pasajeros"].'  </b></u>  PASAJEROS.
...
/**************************************
 *
 *  FMTO ENTRADA MIGRACION PAG 16  >>>>> MAKE AVO
 *
 **************************************/

#$pdf->addJpegFromFile("migra.jpg",'5','50','590','');

$pdf->addText(62,810,12,'BUQUE: <b>'.$row["vessel"].'</b>');
$pdf->addText(47,795,12,'BANDERA: <b>'.$row["flag"].'</b>');
$pdf->addText(22,780,12,'TRIPULANTES: <b>'.$row["allmembers"].'</b>');
$pdf->addText(17,765,12,'PROCEDENCIA: <b>'.$row["lastport"].'</b>');
$pdf->addText(70,750,12,'HORA: <b>'.$row["etahora"].'</b>');
$pdf->addText(64,735,12,'FECHA: <b>'.$row["eta"].'</b>');
$pdf->addText(17,715,16,'<b>TWIN MARINE DE MEXICO</b>');
...
/**************************************
 *
 *  FMTO SALIDA MIGRACION	PAG 17  >>>>> MAKE AVO
 *
 **************************************/
...
$pdf->addText(47,795,12,'BANDERA: <b>'.$row["flag"].'</b>');
$pdf->addText(22,780,12,'TRIPULANTES: <b>'.$row["allmembers"].'</b>');
$pdf->addText(51.5,765,12,'DESTINO: <b>'.$row["etfnextport"].'</b>');
$pdf->addText(70,750,12,'HORA: <b>'.$row["etfhora"].'</b>');
$pdf->addText(64,735,12,'FECHA: <b>'.$row["etf"].'</b>');
$pdf->addText(17,715,16,'<b>TWIN MARINE DE MEXICO</b>');
...
/**************************************
 *
 *  FICHA ENTRADA MIGRACION	PAG 18  >>>>> MAKE AVO
 *
 **************************************/

$pdf->addJpegFromFile("migra.jpg",'5','50','590','');
$pdf->addText(15,564,10,'TWIN MARINE DE MEXICO SA DE CV Y/O '.$row["razonsocial"]);
...
/**************************************
 *
 *  FICHA SALIDA MIGRACION	PAG 19  >>>>> MAKE AVO
 *
 **************************************/
...
#    $pdf->addText(545-$x,$renglon,10,number_format( $pesototal, 3, ".",","));
    ...
    for($i=0; $i<strlen($recibidor[$j]); $i++) {
		...
		$pdf->ezText(utf8_decode($bufx.' - '.number_format( $pesototal, 3, ".",",").' MT'), 12, array('justification'=>'center'));
