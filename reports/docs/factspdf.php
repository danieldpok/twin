...
$query = "select * from operaciones where idop='".$id."'";
...
$pdf->setColor(.1,.1,.1);
$pdf->addText(297.5,740.5,20,'<b>STATEMENT OF FACTS</b>');

$pdf->setColor(1,1,1);
$pdf->addText(297,740,20,'<b>STATEMENT OF FACTS</b>');
...
//---REPORTE GENERAL DE DESCARGA POR RECIBIDOR ---- DAKA TECHNOLOGY---- PROP. INTELECTUAL DANIEL A. KENNEDY AYALA-----
...
for($i=0; $i<=count($fechax); $i++) {
    ...
    $query = "select fecha, hinicial, hfinal, fact from computotiempo where idop='".$id."' and fecha='".$fechax[$i]."' and tipo='ARRIVAL MANEUVERS' order by hinicial";
    $result = mysql_query($query, $link);
    $buf=0;
    ...
    ////////////OPERATIONAL
    $query = "select fecha, hinicial, hfinal, fact from computotiempo where idop='".$id."' and fecha='".$fechax[$i]."' and tipo='OPERATIONAL' order by hinicial, hfinal";
    $result = mysql_query($query, $link);
    $buf=0;
    $date2=false;
    while($row = mysql_fetch_array($result)) {
    ...
    ////////////STOP IDLE TIMES
    $query = "select fecha, hinicial, hfinal, fact from computotiempo where idop='".$id."' and fecha='".$fechax[$i]."' and tipo='STOP/IDLE TIME' order by hinicial, hfinal";
    $result = mysql_query($query, $link);
    $buf=0;
    $date2=false;
    while($row = mysql_fetch_array($result)) {
	...
	//SAILING INFORMATION
$computotiempo="";
	$query = "select fecha, hinicial, hfinal, fact from computotiempo where idop='".$id."' and fecha='".$fechax[$i]."' and tipo='SAILING INFORMATION' order by hinicial, hfinal";
    $result = mysql_query($query, $link);
    $buf=0;
    $date2=false;
    while($row = mysql_fetch_array($result)) {
...
$arg="<b>Powered by    <c:alink:www.twinmarinedemexico.com>TWIN MARINE DE MEXICO, S.A. DE C.V.</c:alink>       <<Copyright � 2013 All Rights Reserved></b>";
