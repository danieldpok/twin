...
$query = "select * from ".$table." where idop='".$id."'";
//echo $query;
$link = Conectar();
...
/////////////ESTADITICA FINALES
            ...
            /////ANALIZA LOS CONCEPTOS
                $query="select fecha, hinicial, hfinal, timepercent, fact, tipo from computotiempox where idop='$id' and fecha='".$fechax[$i]."' and fecha>='$timetocount1' order by hinicial";
                //echo $query;
...
<body bgcolor="#ffffff" >
        ...
        <table width="967" border="1" cellspacing="2" cellpadding="0">
          <tr>
            ...
            for($i=0; $i<count($fechax); $i++) {
            /////ANALIZA LOS CONCEPTOS
                $query="select fecha, hinicial, hfinal, timepercent, fact, tipo, remarks from computotiempox where idop='$id' and fecha='".$fechax[$i]."' and fecha>='$timetocount1' order by hinicial";
                //echo $query;
		    ...
		    <td><table width="607" border="1" cellspacing="2" cellpadding="0">
		      ...
		      <?php
                                                ...
                                                for($i=0; $i<count($recibidor); $i++) {
                                                    //$query = "select recibidor, producto, pesoneto, bl from chargeinformation where idop='$id' and recibidor='$recibidor[$i]' and producto='$producto[$i]'";
													$query = "select distinct recibidor, producto, pesoneto, bl from chargeinformation where idop='$id' and recibidor='$recibidor[$i]'";
                                                    $result = mysql_query($query, $link);
                                                    $bl="";
                                                    $pesoneto=0.000;
                                                    while($row = mysql_fetch_array($result)) {
